...
class PaypalPayment extends StatefulWidget {
  ...
  const PaypalPayment({super.key, required this.onFinish, required this.price, required this.currencyCode, required this.title, required this.description});
    ...
    if (checkoutUrl.isNotEmpty) {
      return WillPopScope(
        ...
        child: Scaffold(
          ...
          body: isLoading ? Center(child: Constant.loader()) : WebViewWidget(controller: controller!),
        ),
      );
    } else {
      return WillPopScope(
  ...
  void showAlertDialog(BuildContext contextt) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const TextCustom(title: "Alert!"),
          content: const TextCustom(title: "Would you like to cancel this payment?"),
          actions: [
            TextButton(
              child: const TextCustom(title: "Continue to Paypal"),
              onPressed: () {
                Navigator.of(context).pop();
              },
            ),
            TextButton(
              child: const TextCustom(title: "Cancel Payment"),
              onPressed: () {
                Navigator.of(context).pop();
