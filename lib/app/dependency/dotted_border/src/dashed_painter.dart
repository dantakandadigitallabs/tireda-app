import 'package:eSellify/app/dependency/dotted_border/src/dotted_border_options.dart';
...
class DashedPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size originalSize) {
    ...
    size = Size(originalSize.width - padding.horizontal, originalSize.height - padding.vertical);
    ...
    Path path = customPath?.call(size) ?? _getPath(size);
    ...
    canvas.drawPath(dashPath(path, dashArray: CircularIntervalList(dashPattern), animationValue: animation?.value), paint);
  }
...
extension _SizeToPathExtension on Size {
  ...
  Path toRectangularPath() => Path()..addRect(Rect.fromLTWH(0, 0, width, height));

  /// Returns a rounded rectangular path of the given size and radius.
  Path toRoundedRectangularPath(Radius radius) => Path()..addRRect(RRect.fromRectAndRadius(Rect.fromLTWH(0, 0, width, height), radius));
  ...
  Path toCirclePath() => Path()
    ..addRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(width > shortestSide ? (width - shortestSide) / 2 : 0, height > shortestSide ? (height - shortestSide) / 2 : 0, shortestSide, shortestSide),
